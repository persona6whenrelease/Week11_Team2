...
void UHopMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction& ThisTickFunction)
{
	...
	else
	{
		...
		if (bShouldApplyHop)
		{
			const float PrevPhase = GetHopPhase(HopElapsedTime, Clamp(HopFrequency, 0.0f, KMaxReasonableHopFrequency));
			...
			// 새로운 홉 사이클이 시작되는 시점 감지
			if (Phase < PrevPhase || (PrevPhase <= 0.001f && Phase > 0.001f))
			{
				// 중복 재생 방지 (사운드가 이미 재생 중이면 무시)
				if (!FSoundManager::Get().IsEffectPlaying(CrossyAudioIds::Jump))
				{
					FSoundManager::Get().PlayEffect(CrossyAudioIds::Jump);
				}
			}
		}
