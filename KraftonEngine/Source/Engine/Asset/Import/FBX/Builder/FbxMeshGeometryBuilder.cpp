...
namespace
{
    ...
    template <size_t Count> void HashArray(size_t &Seed, const std::array<uint32, Count> &Values)
    {
    ...
    struct FFbxStaticVertexKey
    {
        ...
        {
            return EqualArray(Position, Other.Position) && EqualArray(Normal, Other.Normal) &&
                   EqualArray(Color, Other.Color) && EqualArray(UV, Other.UV) &&
    ...
    struct FFbxSkeletalVertexKey
    {
        ...
        {
            return EqualArray(Position, Other.Position) && EqualArray(Normal, Other.Normal) &&
                   EqualArray(UV, Other.UV) && EqualArray(Tangent, Other.Tangent) &&
                   EqualArray(BoneIDs, Other.BoneIDs) && EqualArray(BoneWeights, Other.BoneWeights);
        }
    };
    ...
    FFbxStaticVertexKey MakeStaticVertexKey(const FNormalVertex &Vertex)
    {
        ...
        Key.Position = {FloatToBitKey(Vertex.pos.X), FloatToBitKey(Vertex.pos.Y),
                        FloatToBitKey(Vertex.pos.Z)};
        Key.Normal = {FloatToBitKey(Vertex.normal.X), FloatToBitKey(Vertex.normal.Y),
                      FloatToBitKey(Vertex.normal.Z)};
        Key.Color = {FloatToBitKey(Vertex.color.X), FloatToBitKey(Vertex.color.Y),
                     FloatToBitKey(Vertex.color.Z), FloatToBitKey(Vertex.color.W)};
        Key.UV = {FloatToBitKey(Vertex.tex.X), FloatToBitKey(Vertex.tex.Y)};
        Key.Tangent = {FloatToBitKey(Vertex.tangent.X), FloatToBitKey(Vertex.tangent.Y),
                       FloatToBitKey(Vertex.tangent.Z), FloatToBitKey(Vertex.tangent.W)};
        return Key;
    }
    ...
    uint32 FindOrAddStaticVertex(
        const FNormalVertex &Vertex, TArray<FNormalVertex> &Vertices,
    ...
    uint32 FindOrAddSkeletalVertex(const FSkeletalVertex &Vertex, TArray<FSkeletalVertex> &Vertices,
                                   std::unordered_map<FFbxSkeletalVertexKey, uint32,
                                                      FFbxSkeletalVertexKeyHasher> &VertexToIndex)
    {
    ...
    void LogUVStatsIfEnabled(const FFbxMeshMeta &MeshMeta, FbxMesh *Mesh,
                             const FFbxGeometryBuildStats &Stats)
    {
        FbxStringList UVSetNames;
        if (Mesh)
        {
        ...
        UE_LOG("[FBXImporter] Mesh UV sets. MeshId=%d Node=%s Count=%d First=%s Names=%s "
               "PreferredMaterialUVSets=%s",
               MeshMeta.MeshId, MeshMeta.SourceNodePath.c_str(), UVSetNames.GetCount(),
               FirstUVSetName ? FirstUVSetName : "<null>", UVSetNameList.c_str(),
               PreferredUVSetNameList.c_str());

        UE_LOG("[FBXImporter] UV summary. MeshId=%d Node=%s UVSets=%d Preferred=%s Vertices=%d "
               "PreferredOK=%d "
               "SetFallbackOK=%d ManualOK=%d Default=%d GetPolygonVertexUVFailed=%d Unmapped=%d "
               "UVZero=%d UVNonZero=%d "
               "UVMin=(%.6f, %.6f) UVMax=(%.6f, %.6f) FirstNonZero=(%.6f, %.6f)",
               MeshMeta.MeshId, MeshMeta.SourceNodePath.c_str(), UVSetNames.GetCount(),
               PreferredUVSetNameList.c_str(), Stats.UVZeroCount + Stats.UVNonZeroCount,
               Stats.UVReadStats.PreferredSuccessCount, Stats.UVReadStats.UVSetFallbackSuccessCount,
               Stats.UVReadStats.ManualElementFallbackSuccessCount,
               Stats.UVReadStats.DefaultUVCount, Stats.UVReadStats.GetPolygonVertexUVFailedCount,
               Stats.UVReadStats.UnmappedCount, Stats.UVZeroCount, Stats.UVNonZeroCount,
               Stats.UVMin.X, Stats.UVMin.Y, Stats.UVMax.X, Stats.UVMax.Y, Stats.FirstNonZeroUV.X,
               Stats.FirstNonZeroUV.Y);
    }

    void TransformSkeletalVertexToAssetSpace(FSkeletalVertex &Vertex,
                                             ...
                                             bool             bFlipHandedness)
    {
        ...
        Vertex.normal = NormalizeSafe(MeshToAssetBindMatrix.TransformVector(Vertex.normal),
                                      FVector(0.0f, 0.0f, 1.0f));

        const FVector TangentDirection =
            NormalizeSafe(MeshToAssetBindMatrix.TransformVector(
                              FVector(Vertex.tangent.X, Vertex.tangent.Y, Vertex.tangent.Z)),
                          FVector(1.0f, 0.0f, 0.0f));
        Vertex.tangent =
            FVector4(TangentDirection, bFlipHandedness ? -Vertex.tangent.W : Vertex.tangent.W);
    }

    void TransformStaticVertexToAssetSpace(FNormalVertex &Vertex,
                                           const FMatrix &MeshToAssetBindMatrix,
                                           bool           bFlipHandedness)
    {
        Vertex.pos = MeshToAssetBindMatrix.TransformPositionWithW(Vertex.pos);
    ...
    FString GetMaterialSlotName(const FFbxMeshMeta &MeshMeta, int32 MaterialIndex)
    {
        return IsValidIndex(MeshMeta.MaterialSlotNames, MaterialIndex)
                   ? FbxMaterialImportUtils::NormalizeMaterialSlotName(
...
namespace FbxMeshGeometryBuilder
{
        ...
        FFbxSkinnedMeshPart                                 &OutPart)
    {
        ...
        std::unordered_map<FFbxSkeletalVertexKey, uint32, FFbxSkeletalVertexKeyHasher>
            VertexToIndex;
        ...
        if (bFlipWinding)
        {
            UE_LOG("[FBXImporter] Mirrored skeletal mesh transform detected; flipping winding. "
                   "MeshId=%d Node=%s",
        ...
        for (int32 PolyIndex = 0; PolyIndex < PolygonCount; ++PolyIndex)
        {
            ...
            {
                UE_LOG("[FBXImporter] Polygon has more than 3 vertices after preprocess; using fan "
            ...
            const char *PreferredUVSetName =
                IsValidIndex(MeshMeta.MaterialUVSetNames, MaterialSlotIndex) &&
            ...
            for (int32 CornerIndex = 0; CornerIndex < PolySize; ++CornerIndex)
            {
                ...
                Vertex.tex = FBXUtil::ReadUV(Mesh, PolyIndex, CornerIndex, ControlPointIndex,
                                             PolygonVertexCounter, PreferredUVSetName,
                                             &GeometryStats.UVReadStats);
                Vertex.tex.V = 1.0f - Vertex.tex.V;
                Vertex.tangent =
                    FBXUtil::ReadTangent(Mesh, ControlPointIndex, PolygonVertexCounter);
                ...
                const uint32 VertexIndex =
                    FindOrAddSkeletalVertex(Vertex, OutPart.Vertices, VertexToIndex);
            ...
            {
                AppendTriangleIndices(SectionIndices, PolygonVertexIndices[0],
                                      PolygonVertexIndices[i], PolygonVertexIndices[i + 1],
                                      bFlipWinding);
            }
        }
        ...
        for (int32 MaterialIndex : MaterialIndices)
        {
            ...
            OutPart.Indices.insert(OutPart.Indices.end(), SectionIndices.begin(),
                                   SectionIndices.end());
            OutPart.Sections.push_back(Section);
        }

        LogUVStatsIfEnabled(MeshMeta, Mesh, GeometryStats);
        UE_LOG("[FBXImporter] Skeletal vertex dedup. MeshId=%d Node=%s SourceCorners=%d "
               "UniqueVertices=%u Reused=%d",
               MeshMeta.MeshId, MeshMeta.SourceNodePath.c_str(), SourceVertexCount,
               static_cast<uint32>(OutPart.Vertices.size()), ReusedVertexCount);
                                 ...
                                 FStaticMesh &OutMesh)
    {
        ...
        if (bFlipWinding)
        {
            UE_LOG("[FBXImporter] Mirrored static mesh transform detected; flipping winding. "
                   "MeshId=%d Node=%s",
                   MeshMeta.MeshId, MeshMeta.SourceNodePath.c_str());
        }
        ...
        for (int32 PolyIndex = 0; PolyIndex < PolygonCount; ++PolyIndex)
        {
            ...
            for (int32 CornerIndex = 0; CornerIndex < PolySize; ++CornerIndex)
            {
                ...
                const uint32 VertexIndex =
                    FindOrAddStaticVertex(Vertex, OutMesh.Vertices, VertexToIndex);
        ...
        for (int32 MaterialIndex : MaterialIndices)
        {
            ...
            OutMesh.Indices.insert(OutMesh.Indices.end(), SectionIndices.begin(),
                                   SectionIndices.end());
            OutMesh.Sections.push_back(Section);
        }

        LogUVStatsIfEnabled(MeshMeta, Mesh, GeometryStats);
        UE_LOG("[FBXImporter] Static vertex dedup. MeshId=%d Node=%s SourceCorners=%d "
               "UniqueVertices=%u Reused=%d",
               MeshMeta.MeshId, MeshMeta.SourceNodePath.c_str(), SourceVertexCount,
               static_cast<uint32>(OutMesh.Vertices.size()), ReusedVertexCount);
        OutMesh.CacheBounds();
