/**
 * StaticMesh LOD ?앹꽦???꾪븳 硫붿떆 媛꾩냼???뚭퀬由ъ쬁??援ы쁽?쒕떎.
 *
 * Quadric Error Metrics 湲곕컲 edge collapse瑜??ъ슜?섎ŉ, UV seam?대굹 hard edge ?뚮Ц??媛숈? ?꾩튂?? * 以묐났 ?앹꽦???뺤젏?ㅼ쓣 ?꾩튂 湲곗??쇰줈 臾띠뼱 洹좎뿴???앷린吏 ?딅룄濡?泥섎━?쒕떎. 寃곌낵 硫붿떆???쇨컖???섎?
 * 紐⑺몴 鍮꾩쑉??留욎떠 以꾩씠???뱀뀡 ?⑥쐞??癒명떚由ъ뼹 踰붿쐞媛 ?ㅼ떆 援ъ꽦?????덈룄濡??몃뜳?ㅼ? ?뱀뀡 ?뺣낫瑜? * ?④퍡 媛깆떊?쒕떎.
 */
...
namespace
{
    struct FQuadric
    {
        ...
        FQuadric &operator+=(const FQuadric &O)
        {
            for (int i = 0; i < 10; ++i)
                D[i] += O.D[i];
            return *this;
        }
        FQuadric operator+(const FQuadric &O) const
        {
            FQuadric R = *this;
            R += O;
            return R;
        ...
        double Eval(double x, double y, double z) const
        {
            return D[0] * x * x + 2 * D[1] * x * y + 2 * D[2] * x * z + 2 * D[3] * x +
                   D[4] * y * y + 2 * D[5] * y * z + 2 * D[6] * y + D[7] * z * z + 2 * D[8] * z +
                   D[9];
        }
    ...
    void WeldVertices(const TArray<FNormalVertex> &InVerts, const TArray<uint32> &InIndices,
                      TArray<FNormalVertex> &OutVerts, TArray<uint32> &OutIndices,
                      TArray<uint32> &OutUnweldMap)
    {
        ...
        for (uint32 i = 0; i < N;)
        {
            ...
            while (i < N)
            {
                ...
                if (std::abs(PC.X - PR.X) > Eps || std::abs(PC.Y - PR.Y) > Eps ||
                    std::abs(PC.Z - PR.Z) > Eps)
                    break;
                WeldMap[Cur] = NewIdx;
...

    {
        ...
        std::vector<bool> IsBnd(NV, false);
        {
            ...
            for (uint32 f = 0; f < NT; ++f)
            {
                ...
                for (int j = 0; j < 3; ++j)
                {
                    uint32 a = t[j], b = t[(j + 1) % 3];
                    if (a > b)
                        std::swap(a, b);
                    HEs.push_back({a, b});
                }
            }
            std::sort(HEs.begin(), HEs.end(), [](const HE &X, const HE &Y)
                      { return X.A < Y.A || (X.A == Y.A && X.B < Y.B); });
            for (size_t i = 0; i < HEs.size();)
            {
                size_t j = i + 1;
                while (j < HEs.size() && HEs[j].A == HEs[i].A && HEs[j].B == HEs[i].B)
                    ++j;
                if (j - i == 1)
                {
                    IsBnd[HEs[i].A] = true;
        ...
        for (uint32 f = 0; f < NT; ++f)
        {
            const FVector &P0 = V[I[f * 3]].pos, &P1 = V[I[f * 3 + 1]].pos,
                          &P2 = V[I[f * 3 + 2]].pos;
        ...
        auto CalcCost = [&](uint32 v0, uint32 v1) -> FCandidate
        {
            ...
            if (bSolved)
            {
                ...
                if (FVector::Distance(Opt, P0) > EL * 2.0f ||
                    FVector::Distance(Opt, P1) > EL * 2.0f)
                    bSolved = false;
                else
                {
                    C.OptPos = Opt;
                    C.Cost = (float)Qs.Eval(ox, oy, oz);
                }
            }
            if (!bSolved)
            {
                FVector Mid = {(P0.X + P1.X) * 0.5f, (P0.Y + P1.Y) * 0.5f, (P0.Z + P1.Z) * 0.5f};
                double  E0 = Qs.Eval(P0.X, P0.Y, P0.Z), E1 = Qs.Eval(P1.X, P1.Y, P1.Z),
                       EM = Qs.Eval(Mid.X, Mid.Y, Mid.Z);
                if (E0 <= E1 && E0 <= EM)
                {
                    C.OptPos = P0;
                    C.Cost = (float)E0;
        ...
        while (CurT > TargetT && !Heap.empty() && MaxIter-- > 0)
        {
            ...
            if (!VA[C.V0] || !VA[C.V1])
                continue;
            if (C.Version != VVer[C.V0] + VVer[C.V1])
                continue;
            ...
            for (int pass = 0; pass < 2 && !bFlip; ++pass)
            {
                ...
                for (uint32 fi : VF[Src])
                {
                    ...
                    FVector NP0 = (i0 == Src) ? NewPos : OP0, NP1 = (i1 == Src) ? NewPos : OP1,
                            NP2 = (i2 == Src) ? NewPos : OP2;
...

            {
                ...
                if (NLen > 1e-8f)
                    V[Keep].normal = V[Keep].normal / NLen;
            ...
            std::vector<uint32> RemNeigh = VN[Rem];
        ...
        for (uint32 f = 0; f < NT; ++f)
        {
            ...
            if (A == UINT32_MAX || B == UINT32_MAX || C_ == UINT32_MAX)
                continue;
            if (A == B || B == C_ || A == C_)
                continue;
