...
// UAnimSequence asset을 AnimSequence editor/viewer 공통 인터페이스로 노출한다.
// Notify는 asset에 기록되며, viewer는 이 어댑터를 통해 접근한다.
class FUAnimSequenceDataSource : public IAnimSequenceDataSource
{
  public:
    ...
    const TArray<FAnimNotifyEntry> &GetNotifies() const override
    {
        return CachedNotifyEntries;
    }
